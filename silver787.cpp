...
public:
    ...
    Tile* secondStep() {
        return fullPath.size() < 2 ? firstStep() : fullPath[1];
    }
...
public:
    ...
    optional<string> step_move(Pacman& mypac, PacDestinationT& myPacDestinations, PacDestinationT& theirPacDestinations) {        
        ...
        if (path.empty()) {
            ...
            destType = "PotPel";
            //cerr << " Ran routeToClosestPotentialPellet. path.size:" << path.size() << endl;
        }
        ...
        if (!route.empty()) {
            ...
            cerr << " "; printPath(route.fullPath); cerr << endl;
            ...
            stringstream cmd;
            cmd << "MOVE " << mypac.pacId << " " << dest1->x << " " << dest1->y
            << " " << route.firstPelletTile->x << "," << route.firstPelletTile->y <<  " v" << route.firstPelletTile->pelletValue;
            //cerr << " move " << mypac.pacId << " " << dest1->x << " " << dest1->y << endl; cerr.flush();            
            return cmd.str();
        }
        else {
    ...
    void calculateRouteReward(Route& route) {
