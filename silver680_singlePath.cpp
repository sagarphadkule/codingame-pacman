...
public:
    ...
    optional<string> step_move(Pacman& mypac, PacDestinationT& myPacDestinations, PacDestinationT& theirPacDestinations) {        
        ...
        vector<Tile*> path = pathToClosestVisiblePellet(mypac, theirPacDestinations);
        string destType = "VisPel";
        cerr << " Ran pathToClosestVisiblePellet. path.size:" << path.size() << endl;

        // 2. Also get path to closestPotentialPellet. (TODO)
        if (path.empty()) {
            path = pathToClosestPotentialPellet(mypac, theirPacDestinations);   // old function
            destType = "PotPel";
            cerr << " Ran pathToClosestPotentialPellet. path.size:" << path.size() << endl;
        }
    ...
    vector<Tile*> pathToClosestVisiblePellet(Pacman& pac, PacDestinationT& theirPacDestinations) {
        Tile* source = pac.pos;
        ...
        while (!q.empty()) {
            ...
            // GOAL:
            if(currTile->pelletValue > 0 && tileClaimedByPac(currTile) == nullptr) {    
                vector<Tile*> path;
    ...
    /// Route to closest Potential pellet by BFS on the graph.
    /// If that pellet is claimed by other mypac, then choose another BUT IMP choose one that is on the boundary. (not beyond the first accessible pellet on any path).
    /// Goal Criteria: PelletValue==-1; my other pac on a tile is NOT a blocking tile UNLESS it is very close.
    vector<Tile*> pathToClosestPotentialPellet(Pacman& pac, PacDestinationT& theirPacDestinations) {
        Tile* source = pac.pos;
        auto visibleTilesToThisPac = pac.getVisibleTiles();
        ...
        while (!q.empty()) {
            ...
            // GOAL:
            if(currTile->pelletValue == -1 && tileClaimedByPac(currTile) == nullptr) {
                vector<Tile*> path;
                while (currTile != source) {
                    path.push_back(currTile);
                    currTile = discoveredBy.at(currTile);
    ...
    void calculateRouteReward2(Route& route) {
        ...
        while (i <= last && i < N) {
            Tile* tile = route.fullPath[i];
            totalReward += (2 * tile->getPelletValueAdjusted() - 1) * pow(gamma, stepCount);
            i++; stepCount++;
        }
