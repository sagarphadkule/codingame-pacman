...
public:
    ...
    optional<string> step_move(Pacman& mypac, PacDestinationT& myPacDestinations, PacDestinationT& theirPacDestinations) {        
        ...
        auto rewardRange = pair<double, double>(routes.front().totalReward, routes.back().totalReward);
    ...
    vector<Route> extendPathIntoRouteOfN(const Path& path, int N, Pacman& mypac) {
        
        vector<Route> routes;
        if (path.empty()) return routes;
...

        {
            ...
            Tile* source = path.back();
            queue<Tile*> q;
            unordered_map<Tile*, Tile*> discoveredBy;    // aka visited + parent combined. <child, parent>
            unordered_map<Tile*, int> pathLength;
            q.push(source);
            if (path.size() > 1) {
                discoveredBy.emplace(source, path[path.size()-2]);
            }
            else {
                discoveredBy.emplace(source, mypac.pos);
            }
            ...
            while (!q.empty()) {
                ...
                if(goalCondition) {                   
                    ...
                    while (currTile != source) {
                        route.additionalPath.push_back(currTile);
                        if (currTile->getPelletValueAdjusted() == 10) {
                            route.superPellets.push_back(currTile);
                        }
                        currTile = discoveredBy.at(currTile);
                    }
                    
                    reverse(route.additionalPath.begin(), route.additionalPath.end());                    
                    route.fullPath.insert(route.fullPath.end(), route.additionalPath.begin(), route.additionalPath.end());
                    routes.push_back(move(route));
                }
                else {
                    // REPEAT FOR NEIGHBOURS:
                    // {// DEBUG
                    //     if (mypac.pacId == 2 && currTile->x == 9 && currTile->y == 3) {
                    //         cerr << "   Adding neighbours of (9,3). Source: " << source->toStr() << " pac.pos:" << mypac.pos->toStr() << endl;
                    //     }
                    // }
    ...
    void input() {
        ...
        for (int i = 0; i < this->visiblePelletCount; i++) {         
            ...
            if (value == 10) {
                board.superPelletTiles.push_back(&this->board.tiles.at({x,y}));
            }
        }
