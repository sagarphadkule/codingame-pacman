...
public:
    ...
    optional<string> step_move(Pacman& mypac, PacDestinationT& myPacDestinations, PacDestinationT& theirPacDestinations) {
        ...
        //cerr << " ";
        vector<Route> routes = allPossibleRoutes(mypac, N);
        ...
        cerr << "Total Routes: " << routes.size() << " RewardRange: [" << rewardRange.first << ", " << rewardRange.second << "]" << endl;
    ...
    /// Select best path beyond end of given path some reward system accumulated value in M nodes. Use discounted rewards.
    /// Goal Criteria: m additional steps or dead end.
    vector<Route> allPossibleRoutes(Pacman& mypac, int N) {
...

        {
            ...
            Route startingRoute;            
            startingRoute.fullPath = vector<Tile*>();
            startingRoute.fullPath.push_back(mypac.pos);
            startingRoute.horizon = N;
            ...
            while (!q.empty()) {
                ...
                int pathSize = currPath.size();
                Tile* currTile = currPath.back();
                Tile* prevTile = currPath.size() > 1 ? currPath[currPath.size()-2] : nullptr;
                bool goalCondition = false;
                ...
                if (!currRoute.firstPelletTile && currTile->getPelletValueAdjusted() > 0) {
                    if (tileClaimedByPac(currTile) == nullptr) {
                        currRoute.firstPelletTile = currTile;
                        currRoute.pathUptoFirstPellet = currPath;
                    }
                    else {
                        reward += -10; // Dont take this path.
                        goalCondition = goalCondition || true;
                    }
                }
                ///
